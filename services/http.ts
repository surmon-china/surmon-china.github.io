...
export function getRepositories(): Promise<$TODO[]> {
  return cache.promise({
    ...
    promise() {
      return axios
        .get(
          `https://api.github.com/users/${CONFIG.GITHUB_UID}/repos?per_page=1000`
        )
        .then((response) => response.data);
    },
  });
...
export function getFileContent(filePath: string): Promise<$TODO> {
  return cache.promise({
    ...
    promise() {
      return axios
        .get(
          `https://api.github.com/repos/${CONFIG.GITHUB_UID}/${CONFIG.PROJECT_NAME}/contents/${filePath}?ref=vue2`
        )
        .then((response) => response.data);
    },
  });
...
export function getNPMPackages(): Promise<$TODO> {
  return cache.promise({
    ...
    promise() {
      return axios
        .get(
          `https://registry.npmjs.com/-/v1/search?text=maintainer:${CONFIG.NPM_UID}`
        )
        .then((response) => response.data.objects);
    },
  });
...
export function getNPMPackagesDownloads(repoName: string): Promise<$TODO> {
  ...
  return cache.promise({
    ...
    promise() {
      return axios
        .get(
          `https://api.npmjs.org/downloads/point/2015-01-10:${today}/${repoName}`
        )
        .then((response) => response.data);
    },
  });
